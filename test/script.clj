...
(prn (sqlite/execute! "/tmp/foo.db" ["insert into foo (the_text, the_int, the_real, the_blob) values (?,?,?,?)" "foo" 1 3.14 "foo"]))
(def results (sqlite/query! "/tmp/foo.db" ["select * from foo"]))
(prn results)

(deftest results-test
  (is (= [{:the_int 1, :the_real 3.14, :the_text "foo" :the_blob "foo"}] results)))
